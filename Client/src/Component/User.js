...
function User(props) {
  ...
  const SetIMG = async (formData) => {
    const url = `${process.env.REACT_APP_API_URL || "http://localhost:8001/api"}/user/updateimg`;
...
const handelPasswordSubmit = async (e) => {
  ...
  const url = `${process.env.REACT_APP_API_URL || "http://localhost:8001/api"}/user/updatepassword`;
  const response = await fetch(url, {
    method: "POST",
    headers: {
              ...
              <div className="card-body p-4">
                <div className="d-flex">
                  <div>
                    ...
                    <form action={`${process.env.REACT_APP_API_URL || "http://localhost:8001/api"}/user/updateimg`} method="post" encType="multipart/form-data" className='img-form' >
